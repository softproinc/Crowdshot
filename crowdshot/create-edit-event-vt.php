...
if (isset($_POST['save_vt'])) {
	if (isset($_POST['vt_id']) && isset($_POST['vt_status']) && $_POST['vt_status'] &&
		...
		isset($_POST['vt-block-12-content-02-cta-name']) && $_POST['vt-block-12-content-02-cta-name'] && isset($_POST['vt-block-12-content-03-cta-date-time']) && $_POST['vt-block-12-content-03-cta-date-time'] && isset($_POST['vt-block-12-content-04-cta-location']) && $_POST['vt-block-12-content-04-cta-location'] && isset($_POST['vt-block-12-content-05-cta-url']) && $_POST['vt-block-12-content-05-cta-url'] && isset($_POST['vt-block-12-content-06-cta-url-label']) && isset($_POST['vt-block-12-content-07-cta-logo-text-before']) && isset($_POST['vt-block-12-content-08-cta-logo-text-after']) &&
		...
		if ($_POST['vt-block-12-content-06-cta-url-label'] || $_POST['vt-block-12-content-07-cta-logo-text-before'] || $_POST['vt-block-12-content-08-cta-logo-text-after']) {
			...
			$no_error      = FALSE;
			$error_message = 'You must provide either call-to-action link label, text before logo button, or text after logo button. Click <a href="create-edit-event-vt.php?' . (isset($_POST['vt_id']) && $_POST['vt_id'] ? 'vt' : 'event') . '_id=' . (isset($_POST['vt_id']) && $_POST['vt_id'] ? $_POST['vt_id'] : $_POST['event_id']) . '">here</a> to try again.';
	...
	if ($no_error && $page_mode == 'create') {
		if ($vt_album_cover && $event && $event_logo && !empty($event_photos) && (count($event_photos) == $number_of_photos_required_for_vt) && $vt_cta_background_image && !empty($potential_music)) {
			...
			$vt_properties_associative_array['timeline'][] = array('sequence' => 17, 'type' => 'block',      'is_locked' => FALSE, 'properties' => array('type' => 'shot',                                                                                                                                 'content' => array(array('type' => 'image', 'is_locked' => TRUE,  'asset_id' => $event_photos[6]['id']))));
			$vt_properties_associative_array['timeline'][] = array('sequence' => 18, 'type' => 'transition', 'is_locked' => TRUE,  'properties' => array('type' => 'wipe'));
			$vt_properties_associative_array['timeline'][] = array('sequence' => 19, 'type' => 'block',      'is_locked' => FALSE, 'properties' => array('type' => 'shot',                                                                                                                                 'content' => array(array('type' => 'image', 'is_locked' => TRUE,  'asset_id' => $event_photos[7]['id']))));
